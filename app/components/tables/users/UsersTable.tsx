...
import { Search, MoreHorizontal, Check, Clock, X, ChevronLeft, ChevronRight } from "lucide-react";
  ...
  return (
    <div className="bg-white rounded-3xl shadow-sm border border-gray-200 overflow-hidden">
      ...
      <div className="hidden lg:block overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-4 py-4 text-left text-xs font-medium text-gray-500">User</th>
              <th className="px-4 py-4 text-left text-xs font-medium text-gray-500">User ID</th>
              <th className="px-4 py-4 text-left text-xs font-medium text-gray-500">Role</th>
              <th className="px-4 py-4 text-left text-xs font-medium text-gray-500">Contact</th>
              <th className="px-4 py-4 text-left text-xs font-medium text-gray-500">Email</th>
          ...
          <tbody className="divide-y divide-gray-200">
            {paginatedUsers.map((user) => (
              <tr key={user.id} className="hover:bg-gray-50">
                <td className="px-4 py-4 cursor-pointer" onClick={() => handleViewUser(user)}>
                ...
                <td className="px-4 py-4 relative">
                  ...
                  {openDropdown === user.id && (
                    <div className="absolute right-0 mt-2 w-40 bg-white border border-gray-200 rounded-lg shadow-lg z-10" ref={dropdownRef}>
                      <button
                      ...
                      <button className="block w-full text-left px-4 py-2 text-sm hover:bg-gray-50 text-gray-700" onClick={() => setOpenDropdown(null)}>
                        Edit User
                      </button>
                      <button className="block w-full text-left px-4 py-2 text-sm hover:bg-gray-50 text-gray-700" onClick={() => setOpenDropdown(null)}>
                        Delete User
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:hidden gap-4 p-4">
        {paginatedUsers.map((user) => (
          <div key={user.id} className="border border-gray-200 rounded-xl p-4 relative">
            <div className="flex justify-between items-start mb-3">
              <div className="flex items-center gap-3 cursor-pointer" onClick={() => handleViewUser(user)}>
                <img className="h-14 w-14 rounded-lg object-cover" src={user.image} alt={user.name} />
                <div>
              ...
              <div className="relative" ref={dropdownRef}>
                <button onClick={() => setOpenDropdown(openDropdown === user.id ? null : user.id)} className="p-2 text-gray-400 hover:text-gray-600">
                  <MoreHorizontal size={16} />
                </button>
                {openDropdown === user.id && (
                  <div className="absolute right-0 mt-2 w-40 bg-white border border-gray-200 rounded-lg shadow-lg z-10">
                    <button className="block w-full text-left px-4 py-2 text-sm hover:bg-gray-50 text-gray-700" onClick={() => handleViewUser(user)}>
                      View User
                    </button>
                    <button className="block w-full text-left px-4 py-2 text-sm hover:bg-gray-50 text-gray-700" onClick={() => setOpenDropdown(null)}>
                      Edit User
